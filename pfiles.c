...
Show information about all open files in each process.");
  ...
  do
    {
      errno = 0;
      char *endp;
      long int pid = strtol (argv[remaining], &endp, 0);
      if (*endp != '\0' || pid < 0)
        {
      ...
      if ((pid == LONG_MAX && errno == ERANGE)
          || pid < 0
          || (sizeof (pid_t) < sizeof (long int) && pid > INT_MAX))
        {
      ...
      if (dfd == -1)
        {
          if (errno == EACCES)
            error (0, 0, gettext ("\
insufficient privileges to read information of process %ld"),
                   pid);
          else if (errno == ENOENT)
            {
              if (fd_checked == 0)
                {
                  struct statfs st;
                  if (statfs ("/proc/self", &st) < 0
                      || st.f_type != PROC_SUPER_MAGIC)
                    error (EXIT_FAILURE, errno,
                           gettext ("FATAL: proc filesystem not available"));
            ...
            error (0, errno,
                   gettext ("cannot read proc entry for process %ld"), pid);
      ...
      if (nfds == -1)
        {
          if (errno == EACCES)
            error (0, 0, gettext ("\
insufficient privileges to read file descriptors of process %ld"),
                   pid);
          else
            error (0, errno,
                   gettext ("cannot read file descriptor of process %ld"),
                   pid);
              ...
              if (fdinfo_checked == 0)
                {
                  ...
                  if (stat64 ("/proc/self/fdinfo", &st) < 0)
                    {
                      error (0, 0, gettext ("\
proc filesystem does not provide fdinfo; falling back to non-verbose mode"));
              ...
              if (mode == mode_verbose)
                {
                  error (0, errno, gettext ("cannot get descriptor for %s"),
                         buf);
                  goto next2;
                }
            }
      ...
      int fd;
      {
        ...
        if (fd == -1 || (fp = fdopen (fd, "r")) == NULL)
          {
            error (0, errno, gettext ("cannot get limits of executable %ld"),
                   pid);
            if (fd != -1)
              close (fd);
            goto next3;
      ...
      if (fd == -1)
        {
          error (0, errno, gettext ("cannot read command line of process %ld"),
                 pid);
          goto next3;
        }
...
static error_t
parse_opt (int key, char *arg __attribute__((unused)), struct argp_state *state __attribute__((unused)))
{
  switch (key)
    {
    case 'n':
...
Copyright (C) %s Ulrich Drepper <drepper@gmail.com>.\n\
"), "2012");
...
static void
handle_file (pid_t pid, int proc_dfd, int fdinfo_dfd, const char *fname,
             int fd)
{
  struct stat64 st;
...
cannot get information about descriptor %s of process %ld"),
             fname, (long int) pid);
      return;
    }
              ...
              if (proc_net_unix != (void *) -1)
                {
                  ...
                  struct info_net_unix **u = tfind (&search, &proc_net_unix,
                                                    net_unix_compare);
                  if (u != NULL)
                    {
              ...
              if (proc_net_ip != (void *) -1l)
                {
                  ...
                  struct info_net_ip **u = tfind (&search, &proc_net_ip,
                                                  net_ip_compare);
                  if (u != NULL)
                    {
                      const char *domain;
                      ...
                      const char *protostr = ((*u)->proto == IPPROTO_UDPLITE
                                              ? " IPPROTO_UDPLITE" : "");
                      ...
                      printf ("\t%s%s\n"
                              ...
                              get_sock_name ((*u)->type),
                              domain, protostr,
                              inet_ntop ((*u)->domain, localaddr,
                                         localstr, sizeof (localstr)),
                              localport,
                              domain,
                              inet_ntop ((*u)->domain, remoteaddr,
...
static int
fd_filter (const struct dirent64 *d)
{
  ...
  if (*endp != '\0' || (n == LONG_MAX && errno == ERANGE)
      || n < 0 || (INT_MAX < LONG_MAX && n > INT_MAX))
    return 0;
...
static int
net_unix_compare (const void *p1, const void *p2)
{
  const struct info_net_unix *i1 = (const struct info_net_unix *) p1;
  const struct info_net_unix *i2 = (const struct info_net_unix *) p2;
  ...
  while (! feof_unlocked (fp))
    {
      ...
      unsigned long int type;
      int off;
      int cnt = sscanf (line, "%*x: %*x %*x %*x %lx %*x %llu %n",
                        &type, &inode, &off);
      if (cnt == 2)
        {
...
static int
net_ip_compare (const void *p1, const void *p2)
{
  const struct info_net_ip *i1 = (const struct info_net_ip *) p1;
  const struct info_net_ip *i2 = (const struct info_net_ip *) p2;
...
static void
read_proc_X (const char *fname, int domain, int type, int proto)
{
