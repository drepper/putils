...
#define SNLEN(s) sizeof (s) - 1, s
    ...
    { RLIMIT_RTTIME, 'b', unit_useconds, N_("realtime"),
      N_("Set uninterrupted real-time scheduling limits."),
      SNLEN ("Max realtime timeout") }
#endif
  };
...
Show current process limits or set process limits.");

/* Strings for arguments in help texts.  */
static char args_doc[28 + nlimits];
static void
__attribute__ ((constructor))
args_doc_constr (void)
{
  ...
  while (remaining < argc)
    {
      ...
      if (*endp != '\0' || (pid == ULONG_MAX && errno == ERANGE)
...
cannot retrieve %s limits of process %ld"),
                             gettext (limits[l->limit].name),
                             pid);
                      status = 1;
                      continue;
                    }
              ...
              if (prlimit64 (pid, l->limit, &newlimit, NULL) != 0)
                {
                  ...
                  error (0, errno,
                         ngettext ("cannot set %s limit of process %ld",
            ...
            try_proc:
              ...
              int limitsfd = openat (dfd, "limits",
                                     l->set_hard ? O_RDWR : O_WRONLY);
              if (limitsfd == -1)
                goto no_read_limit;
              ...
              if (! l->set_hard)
                {
                  ...
                  while (1)
                    {
                      ssize_t nr = read (limitsfd, &buf[next],
                                         sizeof (buf) - next);
                      if (nr <= 0)
                        break;
                      next += nr;
...
static rlim64_t
parse_limit (const char *arg, int limidx)
{
  ...
  char *endp;
  errno = 0;
  unsigned long long int val = strtoull (arg, &endp, 0);
  if (val == ULLONG_MAX && errno == ERANGE)
  too_large:
    error (EXIT_FAILURE, 0, gettext ("too large limit value %s"), arg);
      ...
      switch (limits[limidx].unit)
        ...
        case unit_seconds:
          if (endp[0] == 'm' && endp[1] == '\0')
            ...
            {
              unsigned long long int val2 = strtoull (endp + 1, &endp, 0);
              if (*endp != '\0' || val2 >= 60)
                goto invalid;
              if (val > ~0ull / 60)
                goto too_large;
...
static int
charcmp (const void *p1, const void *p2)
{
...
static int
print_file_limits (const char *arg)
{
  assert (sizeof (pid_t) == sizeof (int)
          || sizeof (pid_t) == sizeof (long int));
  char *endp;
...
cannot retrieve limit %s for process %ld: %m\n"),
      ...
      if (lim.rlim_cur == RLIM64_INFINITY)
        ...
        printf ("  %-14llu",
                (unsigned long long int) lim.rlim_cur / scale);
